...
class BertLstmCRF(pl.LightningModule):
    def __init__(self,
                 hidden_size: int,
                 lr: float,
                 weight_decay: float,
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, label_ids=None):
        ...
        hidden_state = self.dropout(last_hidden_state)
        rnn_out, hidden = self.rnn(hidden_state)
    ...
    def configure_optimizers(self):
        ...
        grouped_parameters = [
            ...
            {'params': [p for n, p in self.rnn.named_parameters() if not any(nd in n for nd in no_decay)],
            'lr': self.hparams.lr * 100, 'weight_decay': self.hparams.weight_decay},
            {'params': [p for n, p in self.rnn.named_parameters() if any(nd in n for nd in no_decay)],
